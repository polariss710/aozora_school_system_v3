...
function QuoteModal({ quote, accessToken, onClose, onSaved }: { quote: QuoteRecord | null; accessToken: string; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { setSaving(true); setError(""); try { const payload = { ...form, exchangeRate: exchangeRateText }; quote ? await updateQuote(accessToken, quote.id, payload) : await createQuote(accessToken, payload); onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : "保存失败"); } finally { setSaving(false); } };
