...
The Income and Expense pages read backend API lists, can create or void manual records, and can submit Cash requests; the Cash Request page reads the backend request queue and lets the School side withdraw requested items; the Account Ledger page is still a read-only finance view while account write actions are connected incrementally.
