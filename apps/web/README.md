...
The Income and Expense pages read backend API lists, can create or void manual records, and can submit Cash requests; the Cash Request page reads the backend request queue and lets the School side withdraw requested items; the Cash Inbound page reads backend events and can reverse posted inbound events; the Account Ledger page remains read-only while account write actions are connected incrementally.
