...
- v3 prod 正式运营起点为 `2026-07`，迁移范围只包含业务月 / 结算月为 `2026-07` 及以后且归属青空进学塾的正式运营数据。
- `2026-06` 及以前数据、历史个人名义数据、早期实验链路和旧 personal Cash linkage 默认留在 v2 查询，不迁移到 v3 主链路。
- 迁移筛选必须使用课时日期、业务月份、账单月份或结算月份，不以 `created_at` 判断业务范围。
- 被迁移范围引用的学生、老师、科目和工资规则按引用闭包迁移，即使主数据创建时间早于 2026-07。
- V2 当前 2026 年 7 月尚未生成月结、工资、学费应收、收入和 Cash 链路，因此首批迁移只承接必要主数据与 7 月预定 / 实际课时，下游记录由 V3 原生生成。
- 旧 `school_payment_requests` 不作为 v3 主业务数据迁移。
- 正式边界、校验和切换顺序见 `docs/v3-prod-migration-boundary.md`。

说明：

- 任何扩大迁移范围的个别数据，都必须作为例外单独记录迁移原因、来源、业务月份、校验方式和回滚方式。
