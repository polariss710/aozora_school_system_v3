...
截至 2026-07-06，后端 `apps/api` 已进入主链路第一轮闭环收口阶段，Controller 路由数为 `145`。当前 API 仍以 dev 环境联调为目标，`apps/api/README.md` 已建立第一版 API 契约索引；前端接入前还需要继续细化字段级 request / response、错误提示口径和列表 / 详情 / 抽屉展示字段。
