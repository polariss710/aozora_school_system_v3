...
- 普通教学业务的 V3 prod 正式运营起点为 `2026-07`，学生课程、学生结算、学费账单、老师工资等事实数据原则上只迁移 `2026-07` 及以后。
- 私塾打工 / 外部授课按完整“2026 结算年度”迁移，固定范围为 `2025-12` 至 `2026-11`；按 `year_month` 判断，不按收入、到账或 Cash 交易日期裁剪。
- 私塾打工迁移必须包含历史导入批次、planned / actual 课时、结算、结算明细、收入、个人 Cash 联动事件、仍被引用的 legacy income request 和必要主数据，不得只迁移汇总。
- 被迁移事实引用的学生、老师、科目、账户、工作单位、业务归属和规则按引用闭包迁移，即使主数据创建时间早于模块起点。
- 历史 UUID、导入批次、原始行、snapshot、来源文件哈希、业务归属和 Cash transaction ID 必须保留；历史 personal 归属不得在迁移时统一改写为青空进学塾。
