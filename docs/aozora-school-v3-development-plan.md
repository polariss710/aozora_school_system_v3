...
V3.1 已确认的首个边界（2026-07-26）：

- 报价对象使用独立的“潜在学生”，不直接选择或创建正式 `students` 记录。
- 沿用 V2 报价时手动输入姓名的运营方式；潜在学生姓名是必填展示字段，但不作为全局唯一身份，允许同名记录。
- 潜在学生、报价草稿、合同草稿和正式学生属于四个不同层次。报价草稿不能自动创建正式学生、正式预定课时、学费账单、收入、支出、Cash 请求或账户流水。
- 首批实现建立潜在学生与报价草稿的真实、可审计数据底座。报价草稿可基于保存时的计算快照查看每周计划，并通过浏览器“打印 / 保存 PDF”导出；这两项均为只读输出，不创建正式事实。合同生成、潜在学生转正式学生，以及报价到课程计划 / 正式课时的转换，均保留为后续明确动作，不在首批写入中隐式发生。
- 报价金额、排课频次、课程明细、币种与有效期的保存及计算口径尚未冻结；在规则明确前，不得用前端 demo 金额或自动计算结果作为正式报价事实。
