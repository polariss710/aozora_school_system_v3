...
- 普通教学业务以 `2026-07` 为正式运营起点，学生课时、学生月度结算、学费账单、老师工资等事实数据原则上只迁移业务月 / 结算月为 `2026-07` 及以后。
- 私塾打工 / 外部授课按完整“2026 结算年度”迁移，范围固定为 `2025-12` 至 `2026-11`；该模块例外不得扩大到普通教学业务。
- 主数据不按创建日期截断；被保留事实引用的学生、老师、科目、账户、工作单位、业务归属和规则按引用闭包迁移。
- 私塾打工必须迁移完整课时、结算明细、收入、联动事件和仍被引用的 legacy request 审计链，不能只迁移月度汇总。
- 历史个人名义业务归属如被私塾打工迁移链引用，必须保留原身份但不得开放为 V3 新业务选项，也不得改写为青空进学塾。
...
- `personal` 可继续存在于 schema 和 dev 回归数据中；V3 prod 不全量迁入 V2 历史个人名义业务记录，但被已批准迁移事实链引用的历史归属必须按引用闭包保留。
