...
- v3 prod 以 `2026-07` 为正式运营起点，只迁移业务月 / 结算月为 `2026-07` 及以后且归属青空进学塾的正式运营数据。
- `2026-06` 及以前数据默认保留在 v2 历史查询系统，不迁移到 v3 主链路。
- 主数据不按 `created_at` 简单截断；被 `2026-07+` 正式业务记录引用的学生、老师、科目和规则按引用闭包迁移。
- V2 已统一的 2026 年 7 月预定 / 实际课时迁入 V3 后，月结、账单、工资、收入、支出和 Cash 链路优先由 V3 原生生成。
- 历史个人名义数据、早期实验链路和旧 personal Cash linkage 不进入 v3 prod 主链路。
- 旧 `school_payment_requests` 不进入 v3 主业务链路。
- 如需查看旧历史数据，回 v2 只读系统查询。
...
- `planned_lessons` 是学生学费生成基础；`actual_lessons` 是学生月度结算和老师工资基础。
...
- `personal` 可继续存在于 schema 和 dev 回归数据中，但 V3 prod 不全量迁入 V2 历史个人名义业务记录。
...
- 正常上课、当月内补课完成、跨月补课待后续完成，都不影响学生本月课时费。
- 取消课时会减少学生本月课时费。
- 跨月补课在后续月份登记完成时，只影响补课月份老师工资，不影响当前月或补课月的学生课时费。
- 月度结算生成结算金额和结转金额，结转金额进入下月学费生成逻辑。
