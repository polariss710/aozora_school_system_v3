...
- 标示为“重置”的按钮只恢复筛选控件的默认值（draft），不得改动已应用条件、列表、统计卡片、分页、导出结果或 URL；必须提示“已重置筛选条件；点击查询后刷新结果。”
- 列表分页属于结果视图状态，不写入筛选 URL。普通“重置”必须保留当前页；用户点击“查询”应用新的条件后，结果集、统计与总条数一起刷新，并回到第 1 页。
- “回到本周 / 本月”等日期快捷导航不是普通“重置”。只有页面已有明确产品定义时，才可同时恢复并应用默认日期；新增时必须先确认其是否影响结果与 URL。
- 查询成功应用条件后，URL 查询参数必须反映 applied 条件；普通“重置”不得改写 URL。刷新或直接打开该 URL 时，正式筛选页必须恢复同一页面的 applied 条件与结果；页面导航离开该结果集时应清除该页面的筛选查询参数。
- 新增或改造任何筛选页面时，必须复用现有 draft/applied 模式或等效公共实现，并在验证记录中覆盖：改选项后结果不变、点击查询后结果和 URL 改变、重置后控件恢复但结果和 URL 不变、再次查询后才按默认条件刷新。
