...
1. 先整理文档和业务规则，再写代码。
2. 技术栈已有初步结论，但未进入实现阶段前仍以业务边界、数据边界和迁移原则为优先。
3. 每次讨论后的结论应沉淀到 `docs/`。
4. 涉及生产数据、Cash 联动、账户流水、工资、收入、支出的设计必须先写清楚流程和回滚/审计策略。
5. v2 继续作为稳定运营来源，不在 v3 中直接沿用所有历史页面和 legacy 表。
6. v3 中不迁移不再需要的过渡期功能。
7. v3 当前仍是本地准备目录，不是正式项目文件夹；准备阶段不需要主动查看 `git status`，也不需要反复报告“不是 git 仓库”。
...
- 未明确要求前，不新增前端或后台程序代码。
- 未明确要求前，不创建数据库 migration、RPC、Edge Function 或部署脚本。
...
当前检查结果显示：项目目录尚未初始化为 Git 仓库。

后续建议：

1. 初始化 Git 前，先确认要纳入版本控制的文件范围。
2. `.DS_Store` 等本地系统文件不应纳入版本控制。
3. 初始化后应创建 `.gitignore`。
4. 每次重要文档整理或代码变更应单独提交。
5. 提交信息应说明业务含义，而不只是文件名。
...
- v3 主业务数据迁移范围为 v2 中 2026 年 5 月开始的数据。
