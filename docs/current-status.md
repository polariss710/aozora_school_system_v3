...
- 2026-07-24 账户流水页已接通既有 Staging API 的独立资金动作：可新增手工账户流水、创建同币种内部调拨；手工/更正流水可走专门冲销，内部调拨可整笔作废。收入、支出、Cash 入站、报销、历史导入等来源仍只读或回到各自主链路，前端不提供删除和绕过入口。动作范围、回滚与审计策略见 `docs/account-ledger-operation-contract.md`。
