...
普通教学现已具备 database-free 的受控 snapshot / exclusion manifest 合同与 staging 准备门禁：快照需以 aggregate-only inventory SHA-256 固定来源范围，保留 eligible 主数据 / 课时 / 月结 / 账单 / 收支的来源 UUID 与引用闭包；每条省略链必须与独立 V2 只读 manifest 精确一致。准备命令仅接受仓库外 `600` 文件、显式 v3-staging target 与双重确认，返回 `prepared_not_applied`，不会创建数据库连接、写入行或 Cash request。普通教学 production snapshot 和 persistent importer 仍未建立或执行。

普通教学的 source-side 导出 SQL 也已固定但尚未运行：它采用 `REPEATABLE READ + READ ONLY + ROLLBACK`，仅在未来获得针对普通教学业务行的 source-read 授权后才能执行。该合同会把受影响工资支出、月结关联账单 / 收入及带附件支出排除在 eligible snapshot 外，并生成 V2 只读 omission candidates；范围前 planned 课时只有被范围内 actual 引用时才能作为显式 reference closure 出现。它不连接 V3 staging/prod，也不写入 School / Cash source。
