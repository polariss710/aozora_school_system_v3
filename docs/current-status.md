...
下一阶段需要将同一 rollback-only target applier 接入 staging 凭据并重跑对账，随后再设计持久 apply 的双人确认、preflight、批次回滚和 production 切换条件；在用户继续禁止 production 数据副本的边界下，不读取或导入 production 逐行数据。
