...
- School 新增只读 `/api/cash/payment-batches` 审计接口，Cash 请求页面把聚合批次与普通请求一并展示，可查看 School/Cash batch、transaction、逐条 request / expense、业务归属、金额和同步错误。API 路由已在 dev 部署并返回鉴权保护；前端构建通过，浏览器登录会话恢复后再做人工页面验收。
...
- 老师工资聚合付款与整组拒绝已完成 dev 代码和数据库级验收；Cash dev 静态 v3-6 的真实页面点击、School 聚合审计页面人工验收仍待最新静态部署和登录会话完成。staging / prod 仍需分别应用 School migration、Cash 增量 SQL、环境凭据与真实 E2E。
