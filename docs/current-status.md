...
- 2026-07-25 已将打工课时、打工结算、Cash 请求纳入正式筛选契约：业务月份、授课机构、对应状态 / 结算状态、Cash 方向 / 状态与关键词均从真实记录派生；修改控件只改 draft，只有查询才刷新列表或左右配对、统计、分页与 URL，普通重置只清空控件并保留当前结果 / URL。三页与原有五个正式筛选页统一使用每页 20 条的结果分页。
