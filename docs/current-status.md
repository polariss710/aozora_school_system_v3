...
- 2026-07-25 进度归纳：Staging 已具备 School + Cash 同 project 的完整验收环境，核心教学、学费 / 收据、工资、外部授课、手工收支、账户流水、报销、Cash 单笔 / 聚合 / FX 主链路均已实装并完成对应自动或人工验收；受控历史副本只用于 Staging 只读查看，生产系统持续运行且未被写入、删除、冻结或导入。前端当前 11 个正式查询页（课时、账单、月结、收入、支出、打工课时、打工结算、Cash 请求、账户流水、Cash 入站、报销）都遵守统一 draft / applied、查询触发、重置不查询、URL 只保存 applied 条件、20 条分页的规则。最新默认分支提交为 `fe49d4c`；报价单、合同草稿、课程计划仍是明确标示的签约前 demo，尚未进入 V3.0 主链路写入。
...
- 2026-07-24 已补齐筛选 URL 的恢复边界：上述正式筛选页刷新或直接打开已应用筛选 URL 时，会恢复同一页面和同一 applied 条件；只读取该页面声明的字段，未知页面或字段不会变成页面状态。普通“重置”仍不改写 URL。
- 2026-07-25 已将打工课时、打工结算、Cash 请求、账户流水、Cash 入站、报销管理纳入正式筛选契约：业务月份、授课机构、对应状态 / 结算状态、Cash 方向 / 状态、School 账户 / 流水方向 / 状态、入站类型及垫付账户与关键词均从真实记录派生；修改控件只改 draft，只有查询才刷新列表或左右配对、统计、分页与 URL，普通重置只清空控件并保留当前结果 / URL。六页与原有五个正式筛选页统一使用每页 20 条的结果分页。
- 2026-07-24 已完成正式筛选结果一致性收口：课时管理、学费账单、月度结算、收入记录、支出记录的统计卡片与结果条数均只读取 applied 条件；查询后重置为第 1 页，普通重置保留当前结果、URL 与页码。五页统一使用每页 20 条的结果分页。课时页待补余额仍只允许按学生范围累计，不会被月份、老师、科目或关键词重新定义。
- 2026-07-24 外部授课页面已接通既有 Staging API：打工课时可新增预定、编辑、删除和登记实际课时；打工结算可先预览后锁定、撤销锁定，并从锁定快照生成收入。所有写入仍须由页面操作者明确提交 / 确认，后端继续负责权限、锁定、审计与收入生成后的不可撤销保护；未修改数据库或生产数据。
- 2026-07-24 基础设置页已接通既有 Staging API：业务归属、School 账户、科目与外部授课机构均可在各自分区新增、编辑、归档与恢复。写入继续由既有后端权限、引用检查与审计负责；账户资料编辑不会改写既有流水。本次只改前端，不改数据库或生产数据。
- 2026-07-24 账户流水页已接通既有 Staging API 的独立资金动作：可新增手工账户流水、创建同币种内部调拨；手工/更正流水可走专门冲销，内部调拨可整笔作废。`pending + not_requested` 的手动收入/支出详情也可选择同币种 School 账户直接入账，金额仍由后端从原单据读取，成功后进入 `account_transaction_created`，不能再提交 Cash。Cash 入站、报销、历史导入及其他来源仍只读或回到各自主链路，前端不提供删除和绕过入口。动作范围、回滚与审计策略见 `docs/account-ledger-operation-contract.md`。
...
1. 维持已完成的 11 个正式查询页和核心写入链路，优先处理人工验收发现的真实问题；不为 demo 页面补造数据或假动作。
2. 将 Cash `pending cancel` 与 FX 部分购汇分摊保留为明确限制：它们需要 Cash 合同 / 数据模型决定，不能在 School 前端单方面绕过。
3. 审计中心维持真实只读列表和详情；如要加入关键词或时间范围检索，先冻结 API 查询合同、权限与审计保留策略，再独立实现。

### 阶段 B：V3.1 签约前模块（需先确认是否启动）

- 报价单、合同草稿、学生跟踪 / 招生 CRM 当前都是明确的 demo 或 V2 独立工具，不属于 V3.0 主链路。
- 若启动，先完成草稿模型、权限、导出、审计和“绝不直接创建正式课时”的边界；报价 / 合同到正式课时只能经后续专用转换流程。

### 阶段 C：V3.2 长期课程计划（需先确认业务规则）

- 课程计划应成为长期预定课时生成器的草稿来源，而不是另一套正式事实。
- 必须先确定预览、确认、去重、冲突检查、来源追踪、锁定后修改和批量生成失败回滚规则；完成后才可写入正式预定课时。

### 阶段 D：Prod 建设与切换（不提前执行）

只有生产授权、负责人、冻结窗口、final-delta / freeze Go/No-Go、v3-prod 环境与回滚负责人全部确认后，才依次执行：创建 v3-prod → 空环境安装与隔离验证 → 最终只读 snapshot / delta → 迁移与对账 → 单点切换 → 观察期。禁止长期双写；现行 School / Cash production 在此之前继续独立运行。
