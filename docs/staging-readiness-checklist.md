...
- schema、权限、staging Auth / seed、部署健康、CORS、完整合成业务 E2E、工资 / FX UI、验收事实清理、最终数据库对账与无密钥运营探针已通过。School schema 已提升到 26 个 migrations，并通过回滚式合成迁移验收和零残留复核。每小时 GitHub Actions 调度已进入默认分支，手动运行、定时重试和失败邮件接收已验证；普通教学受控 snapshot、importer 与 rehearsal 已完成，prod 建设仍未开始，因此本文第 10 节完成标准仍未全部通过。
...
截至 2026-07-20，空 staging 的完整合成业务 E2E、必要 UI、精确清理、最终数据库对账和无密钥运营探针已通过，报告已形成。School schema 当前为 26 个 migrations；新增历史迁移审计、普通教学迁移批次 schema、历史已确认支出状态、普通教学主数据 legacy identity 及额外历史 actual→planned 只读链接的 staging 验收均已通过，`anon` / `authenticated` grant 为 0。每小时 GitHub Actions 调度、手动运行、定时重试和失败邮件接收均已验证。受控 production snapshot 的 School 私塾打工、Cash ledger 与普通教学初始副本均已导入 staging，计数、8 条跨系统关联、普通教学 0 Cash request 与三套 importer 幂等复跑均通过；历史已确认记录的只读迁移审计页面也已上线复测。普通教学的复杂工资 / 月结链、附件和 legacy payment request 已批准按 `v2_readonly_retention_v1` 保留 / 省略；历史导入课时在 API 与页面均被明确限制为只读。新增 metadata-only final-delta / freeze Go/No-Go 门禁，但 production 授权、冻结窗口与负责人尚未完成，因此仍不得进入 `v3-prod` 建设。
