...
- schema、权限、staging Auth / seed、部署健康、CORS、完整合成业务 E2E、工资 / FX UI、验收事实清理、最终数据库对账与无密钥运营探针已通过。School schema 已提升到 25 个 migrations，并通过回滚式合成迁移验收和零残留复核。每小时 GitHub Actions 调度已进入默认分支，手动运行、定时重试和失败邮件接收已验证；普通教学受控 snapshot / importer / rehearsal 与 prod 建设仍未完成，因此本文第 10 节完成标准仍未全部通过。
...
- 普通教学只导入 `2026-07+` 中可完整映射的事实；主数据按引用闭包导入。依赖多维工资明细 / 调整或学生月结 adjustment / carryover 的整条链保留 V2 只读；附件与 legacy payment request 按已批准的 omission policy 处理。
- 私塾打工按完整 2026 结算年度 `2025-12` 至 `2026-11` 导入，并保留完整审计链。
- 如使用真实数据副本，必须记录来源快照、脱敏方式、导入批次、行数、哈希及逐链 exclusion manifest；manifest 必须说明每项 V2 只读保留或附件不复制的处理，不得静默漏迁。
...
- 第六轮老师工资真实聚合通过自动与人工验收：JPY 2,100 / 1,800 两条同组工资只生成一条 JPY 3,900 Cash transaction；重放、冲突、同步和只读 UI 正常，人工确认后精确清理为 0。
- 第七轮 CNY→JPY FX 入站通过自动与人工验收：CNY 88.00 confirmed 来源收入匹配一组双向 Cash FX pair，School 创建唯一 JPY 1,800 法人账户流水；callback / sync 重放、冲突、四种 guard 和只读 UI 正常，人工确认后精确清理为 0。
...
截至 2026-07-20，空 staging 的完整合成业务 E2E、必要 UI、精确清理、最终数据库对账和无密钥运营探针已通过，报告已形成。School schema 当前为 25 个 migrations；新增历史迁移审计、普通教学迁移批次 schema、历史已确认支出状态及普通教学主数据 legacy identity 的 staging 验收已通过，普通教学批次表与 `migration_record_audits.core_teaching_batch_id` 均存在，`business_entities` / `students` / `teachers` / `subjects` 的 legacy source identity 有配对与唯一防线，`anon` / `authenticated` grant 为 0。每小时 GitHub Actions 调度、手动运行、定时重试和失败邮件接收均已验证。受控 production snapshot 的 School 私塾打工和 Cash ledger 初始副本已导入 staging，计数、8 条跨系统关联和两套 importer 幂等复跑均通过；历史已确认记录的只读迁移审计页面也已上线复测。普通教学的复杂工资 / 月结链、附件和 legacy payment request 已批准按 `v2_readonly_retention_v1` 保留 / 省略；受控 snapshot / exclusion manifest 与 `prepared_not_applied` staging-only 门禁已完成，下一步才是 persistent importer 与演练。新增 metadata-only final-delta / freeze Go/No-Go 门禁，但 production 授权、冻结窗口与负责人尚未完成，因此仍不得进入 `v3-prod` 建设。
