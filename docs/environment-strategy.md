...
费用估算按 Micro compute 粗略记录：

```text
当前 Cash project           $10
当前 School v1/v2 project   $10
v3 dev project              $10
Pro subscription            $25
Compute credits            -$10
--------------------------------
...
进入测试 / 上线准备阶段后，如保留现有 Cash、现有 School、v3 dev、v3 staging、v3 prod 五个 cloud project：

```text
5 个 Micro project          $50
Pro subscription            $25
Compute credits            -$10
--------------------------------
合计                        约 $65/月
...
以上为当前规划估算，实际以 Supabase billing 页面为准。
...
2. 一键导入 seed data。
3. 一键清空 dev 测试数据。
4. staging 可导入指定月份之后的真实数据副本，例如 2026-05 之后。
