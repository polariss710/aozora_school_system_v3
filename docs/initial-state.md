...
v3 已进入正式工程准备阶段：前端 demo 主框架已确认，后端 skeleton 已建立，dev 数据库已开始接入。
...
- 后端已建立 NestJS skeleton，并已接入 Supabase v3 dev PostgreSQL。
...
1. 建立 PrismaService / Repository 基础访问层。
2. 进入认证权限骨架：登录、用户、角色、权限校验。
3. 继续补主链路第一批数据模型。
