...
- 前端 demo 已开始接真实 dev API：登录页会检查 API health，优先调用 `/api/auth/login`；登录成功后学生管理页和老师管理页可用 token 拉取真实列表，并可调用新增、基础信息编辑、归档和恢复动作；基础设置页可按类型标签页只读拉取业务归属、School 账户、科目和外部授课机构；收入和支出页可拉取真实列表并新增手动记录；账户流水页可只读拉取真实列表。API 不可达时保留 demo fallback。
...
- 未全面连接真实后端 API 到前端；当前完成 health / auth / students / teachers / settings 第一轮接入，完成 income / expenses 列表和手动新增接入，并完成 account transactions 只读列表接入。
- 未导入或迁移 v2 数据。
- 未读取或修改 v2 项目。
- 未执行任何生产数据操作。
...
- 前端 demo 第一轮真实 API 接入：
  ...
  - 收入记录、支出记录页已接 `/api/income`、`/api/expenses` 列表和 `/api/income/manual`、`/api/expenses/manual` 手动新增；账户流水页已接 `/api/accounts/transactions` 只读列表。
...
3. 前端以 students / teachers / settings / finance manual records 为样板，继续把收入 / 支出作废、Cash 请求、Cash 入站和报销页面逐步接入真实 dev API。
