...
- 明确普通教学 `2026-07+` 与私塾打工完整 2026 结算年度的不同迁移口径。
...
- 未建立正式迁移脚本和迁移校验报告。
- 已确认 V3 prod 采用模块化迁移范围：普通教学事实原则上承接 `2026-07+`，私塾打工承接完整 2026 结算年度 `2025-12` 至 `2026-11`，主数据按引用闭包迁移；正式边界记录在 `docs/v3-prod-migration-boundary.md`，执行状态记录在 `docs/current-status.md`。
