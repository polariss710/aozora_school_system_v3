...
迁移边界补充（2026-07-15）：本文件只对齐普通学生与财务链路；因普通教学业务归属规则在 2026-07 收口，该模块事实原则上只承接 `2026-07+`。私塾打工是无普通教学业务归属维度的独立系统，另按完整 2026 结算年度迁移，具体范围见 `docs/v3-prod-migration-boundary.md`。
...
| 生成前只读 preview 和来源明细 | 已对齐 | 预览与正式生成共享后端计算上下文，前端不提交金额 |
| 账单生成后可生成收入 | 已对齐 | 生成收入前保持账单可修正 |
| 基础课时或结转变化后重新计算 | 已对齐 | 来源字段或结转变化时预览明确显示新版本；完全一致时不重复生成 |
...
V2 和 V3 核心口径一致：收据只能从已 Cash 确认的学费收入进入，金额使用 Cash 确认后的权威到账金额，不能自由手填。

V3 已完成阶段 1：

- 收入列表仅对符合条件的 `student_tuition_bill` 收入显示“生成学费收据”。
- `GET /api/income/:id/receipt` 在后端校验学生、账单、收入状态、Cash 状态、Cash 确认记录和有效金额。
- 已存在收入账户流水时优先使用流水金额和日期；否则使用已确认 Cash 请求金额，并优先采用关联 Cash 入站日期。
- 前端只读取收据数据并提供预览及“打印 / 保存 PDF”，不提交或重算金额。
- 阶段 1 只提供权威数据预览和浏览器打印，不直接落收据台账。

V3 已完成阶段 2：

- 新增 `receipt_records` 和 Prisma migration，固定收据编号、开具人、开具时间、权威金额、学生名、业务归属、项目、收款日期、Cash 来源追踪和 PDF 版式元数据。
- `POST /api/income/:id/receipt` 负责幂等开具；同一收入重复请求返回原快照，不重复编号。
- `GET /api/income/:id/receipt` 在未开具时返回实时权威预览，开具后只返回收据快照；另提供历史列表和按收据 ID 读取接口。
- 前端区分“生成学费收据”和“查看学费收据”，只有开具并固定快照后才允许打印 / 保存 PDF。
- 阶段 3 再处理作废 / 重开、多笔 Cash 收款、部分付款和多月合并付款；本阶段一笔收入最多一张收据。
...
1. 已完成学生月度结算前端动作，并验证课时处理状态、锁定、撤销和结转保护。
2. 已完成学费账单 preview、来源明细和版本化重新生成保护；继续验证 dev 部署后的完整链路。
3. 对照现有 Cash 系统确定请求、账户、确认、拒绝、transaction 和回写幂等合同。
4. 已完成学费收据第一阶段：已 Cash 确认收入 -> 权威金额 -> 打印 / 保存 PDF。
5. 已完成学费收据第二阶段：权威预览 -> 幂等开具 -> `receipt_records` 快照台账 -> 历史查看 / 重新打印。
